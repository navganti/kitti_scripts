...
public:
  ...
  Matrix(const int32_t m, const int32_t n,
         const double *val_); // init mxn matrix with values from array 'val'
  ...
  // row/column/matrix
  void getData(double *val_, int32_t i1 = 0, int32_t j1 = 0, int32_t i2 = -1,
               int32_t j2 = -1);
  ...
  // row/column/matrix
  void setVal(double s, int32_t i1 = 0, int32_t j1 = 0, int32_t i2 = -1,
              int32_t j2 = -1);
  ...
  bool
  solve(const Matrix &M,
        double eps = 1e-20); // solve linear system M*x=B, replaces *this and M
  bool lu(int32_t *idx, double &d,
          double eps = 1e-20); // replace *this by lower upper decomposition
