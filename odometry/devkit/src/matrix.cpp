...
void Matrix::getData(double *val_, int32_t i1, int32_t j1, int32_t i2,
                     int32_t j2) {
  if (i2 == -1) {
      i2 = m - 1;
  }
...
Matrix Matrix::getMat(int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
  ...
  if (i1 < 0 || i2 >= m || j1 < 0 || j2 >= n || i2 < i1 || j2 < j1) {
    std::cerr << "ERROR: Cannot get submatrix [" << i1 << ".." << i2 << "] x ["
              << j1 << ".." << j2 << "]"
              << " of a (" << m << "x" << n << ") matrix." << std::endl;
    exit(0);
  }
...
void Matrix::setMat(const Matrix &M, const int32_t i1, const int32_t j1) {
  if (i1 < 0 || j1 < 0 || i1 + M.m > m || j1 + M.n > n) {
    std::cerr << "ERROR: Cannot set submatrix [" << i1 << ".." << i1 + M.m - 1
              << "] x [" << j1 << ".." << j1 + M.n - 1 << "]"
              << " of a (" << m << "x" << n << ") matrix." << std::endl;
    exit(0);
  }
...
void Matrix::setVal(double s, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
  ...
  if (i2 < i1 || j2 < j1) {
    std::cerr << "ERROR in setVal: Indices must be ordered (i1<=i2, j1<=j2)."
         << std::endl;
    exit(0);
  }
...
Matrix Matrix::extractCols(std::vector<int> idx) {
  Matrix M(m, static_cast<int32_t>(idx.size()));
...
Matrix Matrix::diag(const Matrix &M) {
  ...
  std::cerr << "ERROR: Trying to create diagonal matrix from vector of size ("
            << M.m << "x" << M.n << ")" << std::endl;
...
Matrix Matrix::reshape(const Matrix &M, int32_t m_, int32_t n_) {
  if (M.m * M.n != m_ * n_) {
    std::cerr << "ERROR: Trying to reshape a matrix of size (" << M.m << "x"
              << M.n << ") to size (" << m_ << "x" << n_ << ")" << std::endl;
    exit(0);
  }
...
Matrix Matrix::operator-(const Matrix &M) {
  ...
  if (A.m != B.m || A.n != B.n) {
    std::cerr << "ERROR: Trying to subtract matrices of size (" << A.m << "x"
              << A.n << ") and (" << B.m << "x" << B.n << ")" << std::endl;
    exit(0);
  }
...
Matrix Matrix::operator*(const Matrix &M) {
  ...
  if (A.n != B.m) {
    std::cerr << "ERROR: Trying to multiply matrices of size (" << A.m << "x"
              << A.n << ") and (" << B.m << "x" << B.n << ")" << std::endl;
    exit(0);
  }
...
Matrix Matrix::operator/(const Matrix &M) {
  ...
  if (A.m == B.m && A.n == B.n) {
    ...
    std::cerr << "ERROR: Trying to divide matrices of size (" << A.m << "x"
              << A.n << ") and (" << B.m << "x" << B.n << ")" << std::endl;
    exit(0);
  }
}
...
Matrix Matrix::cross(const Matrix &a, const Matrix &b) {
  if (a.m != 3 || a.n != 1 || b.m != 3 || b.n != 1) {
    std::cerr << "ERROR: Cross product vectors must be of size (3x1)"
              << std::endl;
    exit(0);
  }
...
Matrix Matrix::inv(const Matrix &M) {
  if (M.m != M.n) {
    std::cerr << "ERROR: Trying to invert matrix of size (" << M.m << "x" << M.n
              << ")" << std::endl;
    exit(0);
  }
...
bool Matrix::inv() {
  if (m != n) {
    std::cerr << "ERROR: Trying to invert matrix of size (" << m << "x" << n
              << ")" << std::endl;
    exit(0);
  }
...
double Matrix::det() {
  if (m != n) {
    std::cerr << "ERROR: Trying to compute determinant of a matrix of size ("
              << m << "x" << n << ")" << std::endl;
    exit(0);
  }
...
bool Matrix::solve(const Matrix &M, double eps) {
  ...
  if (A.m != A.n || A.m != B.m || A.m < 1 || B.n < 1) {
    std::cerr << "ERROR: Trying to eliminate matrices of size (" << A.m << "x"
              << A.n << ") and (" << B.m << "x" << B.n << ")" << std::endl;
    exit(0);
  }

  // index vectors for bookkeeping on the pivoting
  auto *indxc = new int32_t[m];
  auto *indxr = new int32_t[m];
  auto *ipiv = new int32_t[m];
...
bool Matrix::lu(int32_t *idx, double &d, double eps) {
  if (m != n) {
    std::cerr << "ERROR: Trying to LU decompose a matrix of size (" << m << "x"
              << n << ")" << std::endl;
    exit(0);
  }
  ...
  double *vv = (double *)malloc(
      n * sizeof(double)); // vv stores the implicit scaling of each row.
...
void Matrix::svd(Matrix &U2, Matrix &W, Matrix &V) {
  ...
  for (k = n - 1; k >= 0; k--) {
    for (its = 0; its < 30; its++) {
      ...
      if (its == 29) {
          std::cerr << "ERROR in SVD: No convergence in 30 iterations"
                    << std::endl;
      }
...
void Matrix::allocateMemory(const int32_t m_, const int32_t n_) {
  ...
  val[0] = (double *)calloc(static_cast<size_t>(m * n), sizeof(double));
