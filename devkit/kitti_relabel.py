...
#
# KITTI Segmentation Relabelling Script
# 
# To change labels in the labeled images from one set to another
#
...
def create_label_dict(labels_a, labels_b):
    """Creates an array such that dictionary[labels_a_train_id] === labels_b_train_id"""
    print("Creating labels dictionary...")
    dictionary = {} 
    for label_a in labels_a:
        matching_labels = [label for label in labels_b if label.id==label_a.id]
...
LABELS_MAPPING = create_label_dict(KITTI_LABELS, WAVELAB_LABELS)
...
def relabel_image(image_path):
    ...
    try:
        ...
        new_pixels = map(relabel_pixel, pixels)
        pixdict = {}
        for p in pixels:
            pixdict[p] = 1
